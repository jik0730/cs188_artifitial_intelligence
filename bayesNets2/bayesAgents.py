...
def constructBayesNet(gameState):
    ...
    # Define edges.
    edges = []
    x_pos_to_fh = (X_POS_VAR, FOOD_HOUSE_VAR)
    edges.append(x_pos_to_fh)
    x_pos_to_gh = (X_POS_VAR, GHOST_HOUSE_VAR)
    edges.append(x_pos_to_gh)
    y_pos_to_fh = (Y_POS_VAR, FOOD_HOUSE_VAR)
    edges.append(y_pos_to_fh)
    y_pos_to_gh = (Y_POS_VAR, GHOST_HOUSE_VAR)
    edges.append(y_pos_to_gh)
    for obsVar in obsVars:
        edges.append((FOOD_HOUSE_VAR, obsVar))
        edges.append((GHOST_HOUSE_VAR, obsVar))
    # Define variableDomainsDict.
    variableDomainsDict = {}
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    for obsVar in bayesNet.variableDomainsDict():
        if obsVar not in ['xPos', 'yPos', 'foodHouse', 'ghostHouse']:
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                # Check observation's position and assign corresponding probability.
                # Case 1: Adjacent with food house.
                # Case 2: Adjacent with ghost house.
                # Case 3: Nothing.
                # Position each of position of houses.
                assign_f = assignment[FOOD_HOUSE_VAR]
                assign_g = assignment[GHOST_HOUSE_VAR]
                if assign_f == TOP_LEFT_VAL:
                    x_f, y_f = topLeftPos
                elif assign_f == TOP_RIGHT_VAL:
                    x_f, y_f = topRightPos
                elif assign_f == BOTTOM_LEFT_VAL:
                    x_f, y_f = bottomLeftPos
                else:
                    x_f, y_f = bottomRightPos
                if assign_g == TOP_LEFT_VAL:
                    x_g, y_g = topLeftPos
                elif assign_g == TOP_RIGHT_VAL:
                    x_g, y_g = topRightPos
                elif assign_g == BOTTOM_LEFT_VAL:
                    x_g, y_g = bottomLeftPos
                else:
                    x_g, y_g = bottomRightPos
                # Adjacent positions of food house and ghost house.
                adjacent_f = [[x_f - 1, y_f - 1], [x_f - 1, y_f], [x_f - 1, y_f + 1],
                              [x_f, y_f + 1], [x_f + 1, y_f + 1], [x_f + 1, y_f], [x_f + 1, y_f - 1]]
                adjacent_g = [[x_g - 1, y_g - 1], [x_g - 1, y_g], [x_g - 1, y_g + 1],
                              [x_g, y_g + 1], [x_g + 1, y_g + 1], [x_g + 1, y_g], [x_g + 1, y_g - 1]]
                obsVar_pos = [int(s) for s in re.findall(r'\b\d+\b', obsVar)]
                # Divide by cases.
                if obsVar_pos in adjacent_f:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    wantedFactor = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR],
                                                            evidence, eliminationOrder)

    prob = float("-inf")
    toReturn = 0
    for assignment in wantedFactor.getAllPossibleAssignmentDicts():
        if wantedFactor.getProbability(assignment) > prob:
            prob = wantedFactor.getProbability(assignment)
            toReturn = assignment
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        factor = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VALS, evidence, eliminationOrder)
        for assignment in factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                p_left = factor.getProbability(assignment)
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                p_right = factor.getProbability(assignment)
        leftExpectedValue = 500.0 * p_left - 500.0 * p_right
        rightExpectedValue = 500.0 * p_right - 500.0 * p_left
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        extraEvidence = self.getExplorationProbsAndOutcomes(evidence)
        for dict in extraEvidence:
            expectedValue += dict[0] * max(self.computeEnterValues(dict[1], enterEliminationOrder))

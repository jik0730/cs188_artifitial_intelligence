...
def joinFactors(factors):
    ...
    for s in setsOfConditioned: conditionedList = conditionedList | s
    for s in setsOfUnconditioned: unconditionedList = unconditionedList | s
    ...
    newFactor = Factor(unconditionedList, conditionedList, factor.variableDomainsDict())
    ...
    for factor in factors:
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            ...
            for oldAssignment in factor.getAllPossibleAssignmentDicts():
                ...
                if find:
                    continue
                newFactor.setProbability(assignment, newFactor.getProbability(assignment) *
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        "*** YOUR CODE HERE ***"
        # Combine conditioned/unconditioned variables(sets)
        conditionedList = list(factor.conditionedVariables().difference(eliminationVariable))
        unconditionedList = list(factor.unconditionedVariables().difference(eliminationVariable))
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            ...
            for assign in newFactor.getAllPossibleAssignmentDicts():
                ...
                if find:
                    continue
                newFactor.setProbability(assign, newFactor.getProbability(assign) + factor.getProbability(assignment))
...
def normalize(factor):
    ...
    "*** YOUR CODE HERE ***"
    # Derive total probability which is for denominator.
    ...
    # Finding variables which have only one entry.
    tobeConditioned = []
    for unconditionedVariable in factor.unconditionedVariables():
        if len(variableDomainsDict[unconditionedVariable]) == 1:
            tobeConditioned.append(unconditionedVariable)
    #print tobeConditioned

    # Define new factor.
    conditionedVariable = list(factor.conditionedVariables())
    unconditionedVariable = list(factor.unconditionedVariables())
    for tb in tobeConditioned:
        conditionedVariable.append(tb)
        unconditionedVariable.remove(tb)
    newFactor = Factor(unconditionedVariable, conditionedVariable, variableDomainsDict)

    for assignment in newFactor.getAllPossibleAssignmentDicts():
        newFactor.setProbability(assignment, factor.getProbability(assignment) / totalProbability)

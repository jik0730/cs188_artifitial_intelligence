...
def enhancedFeatureExtractor(datum):
    ...
    "*** YOUR CODE HERE ***"
    newfeatures = np.zeros_like(datum, dtype=int)
    newfeatures[datum > 0] = 1
    ...
    while checkWhiteSpace:
        ...
        queueForBFS.push(initialPoint)
        newfeatures[initialPoint[0]][initialPoint[1]] = 1
        while not queueForBFS.isEmpty():
            x, y = queueForBFS.pop()
            if 0 <= x - 1 <= 27 and 0 <= y <= 27 and newfeatures[x-1][y] == 0:
                queueForBFS.push([x - 1, y])
                newfeatures[x-1][y] = 1
            if 0 <= x + 1 <= 27 and 0 <= y <= 27 and newfeatures[x+1][y] == 0:
                queueForBFS.push([x + 1, y])
                newfeatures[x + 1][y] = 1
            if 0 <= x <= 27 and 0 <= y - 1 <= 27 and newfeatures[x][y-1] == 0:
                queueForBFS.push([x, y - 1])
                newfeatures[x][y-1] = 1
            if 0 <= x <= 27 and 0 <= y + 1 <= 27 and newfeatures[x][y+1] == 0:
                queueForBFS.push([x, y + 1])
                newfeatures[x][y+1] = 1
        count += 1
        checkWhiteSpace, initialPoint = checkIfthereisWhiteSpace(newfeatures)
    ...
    # Concatenating.
    features = np.concatenate([features, whites])

    return features


def checkIfthereisWhiteSpace(feature):
    toReturn = False
    toReturn2 = (0, 0)
    for x in range(28):
        check = 0

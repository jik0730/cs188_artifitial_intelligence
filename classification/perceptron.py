...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                "*** YOUR CODE HERE ***"
                # print input_train_data[i]
                # print self.weights[label_train_data[i]]

                # Declaration parameters.
                feature = input_train_data[i] # f(X)
                correctLabel = label_train_data[i] # W*

                # Derive label that we are guessing. (argmax)
                findLabel = util.Counter()
                for w in self.weights:
                    findLabel[w] = self.weights[w] * feature
                selectedLabel = findLabel.argMax()

                # Update weight, if-else phrase.
                if selectedLabel == correctLabel:
                    continue
                else:
                    if ((self.weights[correctLabel]+feature) * feature
                            > self.weights[correctLabel] * feature):
                        self.weights[correctLabel] += feature
                        self.weights[selectedLabel] -= feature
                    else:
                        self.weights[correctLabel] -= feature
                        self.weights[selectedLabel] += feature
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        explore = self.weights[label].copy()
        for i in range(100):
            toAppend = explore.argMax()
            best100Features.append(toAppend)
            explore[toAppend] = float("-inf")

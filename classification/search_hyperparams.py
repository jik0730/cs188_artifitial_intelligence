...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        "*** YOUR CODE HERE ***"
        Msolvers = solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)
        Msolvers.solve(train_data, train_labels, val_data, val_labels, model)
        a = model.accuracy(val_data, val_labels)
        val_accuracies.append((index, model, (learning_rate, momentum, batch_size),
                               model.accuracy(val_data, val_labels)))
        # if a > 0.7:
        #     return model, (learning_rate, momentum, batch_size)
        index += 1

    best_model = val_accuracies[0][1]
    best_hyperparams = val_accuracies[0][2]
    best_accuracy = val_accuracies[0][3]
    for i, m, h, a in val_accuracies:
        if a > best_accuracy:
            best_model = m
            best_hyperparams = h
            best_accuracy = a

...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        "*** YOUR CODE HERE ***"
        # print loss_tensor, param_vars
        # session = tf.Session()
        # session.run(tf.initialize_all_variables())
        # variable_value = session.run(param_vars[0])
        # print 'Value:\n%r' % variable_value

        #print grad_tensors
        #loss_value = session.run(grad_tensors, feed_dict={param_vars[0]})

        # Just doing calculation with the formula that we saw in lecture.
        for i in range(len(grad_tensors)):
            grad_tensors[i] = self.learning_rate * grad_tensors[i]
            new_tensor = tf.sub(param_vars[i], grad_tensors[i])
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        # Just doing calculation with the formula that we saw in lecture with momentum.
        for i in range(len(grad_tensors)):
            grad_tensors[i] = self.learning_rate * grad_tensors[i]
            updated_vel = self.momentum * vel_vars[i]
            new_vel = tf.sub(updated_vel, grad_tensors[i])
            updates.append((vel_vars[i], new_vel))

            new_tensor = tf.add(param_vars[i], new_vel)
            updates.append((param_vars[i], new_tensor))
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss, u = session.run([loss_tensor, update_ops], feed_dict={placeholders[0]: train_data[0],
                                                                              placeholders[1]: train_data[1]})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: val_data[0],
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        init = tf.initialize_all_variables()
        session.run(init)
        gen_train = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        gen_val = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            g1 = gen_train.next()
            g2 = gen_val.next()
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: g1[0],
                                                             placeholders[1]: g1[1]})
            session.run(update_ops, feed_dict={placeholders[0]: g1[0],
                                               placeholders[1]: g1[1]})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: g2[0],
                                                           placeholders[1]: g2[1]})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        init = tf.initialize_all_variables()
        session.run(init)
        gen_train = MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        gen_val = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)

...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextPos = (nextx, nexty)
                next1, next2, next3, next4 = state[1], state[2], state[3], state[4]
                if nextPos in self.corners:
                    if nextPos == self.corners[0]:
                        ...
                        next4 = True
                cost = 1
                nextState = [nextPos, next1, next2, next3, next4]
                successors.append( ( nextState, action, cost ) )
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0, 4):
        if state[i + 1]:
            continue
        toCompare = abs(corners[i][0] - state[0][0]) + abs(corners[i][1] - state[0][1])
        if toReturn < toCompare:
            toReturn = toCompare
    return toReturn
...
def foodHeuristic(state, problem):
    ...
    for foodpos in foodGrid.asList():
        toCompare = mazeDistance(position, foodpos, problem.startingGameState)
        if toReturn < toCompare:
            toReturn = toCompare

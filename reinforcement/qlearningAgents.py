    ...
    def update(self, state, action, nextState, reward):
        ...
        self.qvalues[(state, action)] = (1 - self.alpha) * self.getQValue(state, action) \
                                        + self.alpha * (reward + self.discount *
                                                        self.getQValue(nextState, self.getPolicy(nextState)))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        #print f
        for key in f.keys():
            toReturn += self.weights[key] * f[key]
        return toReturn
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        nqv = self.getQValue(nextState, self.getPolicy(nextState))
        for key in f.keys():
            self.weights[key] += self.alpha * (reward + (self.discount * nqv) - qv) * f[key]
        return
